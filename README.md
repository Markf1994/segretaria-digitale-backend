...
2. Create a virtual environment and install dependencies (including `httpx`,
   ...
   ```
3. Install WeasyPrint system dependencies (`libpango-1.0-0`,
   `libgdk-pixbuf2.0-0` and `cairo`) using your package manager. PDF generation
   is handled by WeasyPrint and embeds `Logo.png` using the **Aptos** font.
   Install the font from [Google Fonts](https://fonts.google.com/specimen/Aptos)
   or provide it locally.
...
System packages used by WeasyPrint must also be installed and the
...
When deploying on platforms like Render or Railway, install the system
libraries required by WeasyPrint before Python dependencies. A typical build
command is:

```bash
apt-get update && apt-get install -y libpango-1.0-0 libgdk-pixbuf2.0-0 cairo && pip install -r requirements.txt
```

If deploying with Docker, the provided `Dockerfile` installs these packages
