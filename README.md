...
3. Install [wkhtmltopdf](https://wkhtmltopdf.org/downloads.html) at the system
   level (e.g. `apt-get update && apt-get install -y wkhtmltopdf`). PDF
   generation now produces a formatted layout via `wkhtmltopdf` that references
   `Logo.png` and uses the **Aptos** font. Install the font from
...
`wkhtmltopdf` which embeds `Logo.png` and relies on the **Aptos** font. The
...
formatted layout generated by `wkhtmltopdf`, embedding `Logo.png` and requiring
